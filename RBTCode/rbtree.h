/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Robert Schaedler III, Liam Brew
 * Version     : 1.0
 * Date        : 4/15/2020
...
class RedBlackTree : public Tree {
   public:
    ...
    /**
     * Returns the std::max width of the red-black tree, i.e. the largest number of
     * nodes on any level.
     */
   ...
   private:
    ...
    int height(Node<K, V> *node) const {
        return (node == nullptr) ? -1 : 1 + std::max(height(node->left), height(node->right));
    }
    ...
    size_t leaf_count(Node<K, V> *node) const {
        return (node == nullptr) ? 0 : (node->left == nullptr && node->right == nullptr) ? 1 : leaf_count(node->left) + leaf_count(node->right);
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        return (node == nullptr || !(node->left != nullptr || node->right != nullptr)) ? 0 : 1 + internal_node_count(node->left) + internal_node_count(node->right);
    }
    ...
    int diameter(Node<K, V> *node) const {
        // int diameter(Node<K,V> *node, int *height) {
        // TODO: Can we change to avoid computing the height over and over???
        return (node == nullptr) ? 0 : std::max(height(node->left) + height(node->right) + 3, std::max(diameter(node->left), diameter(node->right)));
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        return (node == nullptr) ? 0 : (level == 0) ? 1 : width(node->left, level - 1) + width(node->right, level - 1);
    }
    ...
    size_t null_count(Node<K, V> *node) const {
        return (node == nullptr) ? 1 : null_count(node->left) + null_count(node->right);
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        return (node == nullptr) ? 0 : level + sum_levels(node->left, level + 1) + sum_levels(node->right, level + 1);
    };
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        return (node == nullptr) ? level : sum_null_levels(node->left, level + 1) + sum_null_levels(node->right, level + 1);
    };
};
