...
struct Node {
    ...
    Node(int level, int data) : parent{nullptr}, left{nullptr}, right{nullptr}, level{level}, data(data) {}

    Node(Node *parent, int level, int data) : parent{parent}, left{nullptr}, right{nullptr}, level{level}, data{data} {}
};

Node *lowest_common_ancestor(Node *p, Node *q) {
}

Node *create_bst(int data[], int low, int high, int level) {
    if (low > high) {
        return nullptr;
    }
    int mid = low + (high - low) / 2;
    Node *parent = new Node(level, data[mid]);
    parent->left = create_bst(data, low, mid - 1, ++level);
    if (parent->left != nullptr) {
        parent->left->parent = parent;
    }
    parent->right = create_bst(data, mid + 1, high, ++level);
...
Node *create_bst(int data[], int length) {
    return create_bst(data, 0, length - 1, 0);
}
